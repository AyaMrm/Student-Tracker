...
public boolean deleteEmploiDuTemps(int idEmploiDuTemps) {
 ...
 public boolean addWeeklySchedule (EmploiDuTemps emploi) {
    String[] joursDeSemaine = {"Dimanche", "Lundi", "Mardi", "Mercredi", "Jeudi", "Vendredi", "Samedi"};

    // Vérifier la disponibilité pour chaque jour de la semaine
    for (String jour : joursDeSemaine) {
        emploi.setJour(jour);  // Modifie le jour de l'emploi du temps

        // Vérifier si le nombre de séances dépasse 6 pour ce jour
        if (getNumberOfSeancesForDay(jour) >= 6) {
            System.out.println("Le nombre maximal de séances pour le " + jour + " est atteint.");
            return false;  // Si le nombre de séances dépasse 6, on arrête et retourne false
        }

        // Vérifier les conflits de salle et de professeur
        if (hasTimeConflict(emploi)) {
            System.out.println("Conflit d'horaires pour la salle " + emploi.getSalle() + " et le professeur " + emploi.getIdProf() + " le " + jour);
            return false;  // Si un conflit est trouvé, on arrête et ne l'ajoute pas
        }

        // Ajouter l'emploi du temps pour ce jour
        if (!addEmploiDuTemps(emploi)) {
            System.out.println("Erreur lors de l'ajout de l'emploi du temps pour le " + jour);
            return false;  // Si l'ajout échoue, on retourne false
        }
    }

    // Si aucun conflit n'a été trouvé et tout s'est bien ajouté
    System.out.println("Emploi du temps ajouté pour toute la semaine !");
    return true;
}

private int getNumberOfSeancesForDay(String jour) {
    String query = "SELECT COUNT(*) FROM emploisdutemps WHERE jour = ?";
    try (PreparedStatement statement = connection.prepareStatement(query)) {
        statement.setString(1, jour);

        try (ResultSet resultSet = statement.executeQuery()) {
            if (resultSet.next()) {
                return resultSet.getInt(1);  // Retourne le nombre de séances pour ce jour
            }
        }
    } catch (SQLException e) {
        System.out.println("Erreur lors de la récupération du nombre de séances pour le jour " + jour + ": " + e.getMessage());
    }
