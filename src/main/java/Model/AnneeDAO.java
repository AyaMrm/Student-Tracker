...
public class AnneeDAO {
    ...
    // Méthode pour récupérer les semestres associés à une année (inchangée)
        ...
        if (annee != null) {
            ...
            try (PreparedStatement stmt = connection.prepareStatement(query)) {
                ...
                while (rs.next() && count < 2) {
                    // Récupérer la colonne correcte "moyenneSemestre"
    ...
    // Méthode pour ajouter une année et l’associer à un étudiant
    /*public boolean addAnnee(Annee annee, int idEtudiant) {
        // Vérifier si l’année existe déjà (basé sur anneeScolaire)
        if (anneeExists("idAnnee", annee.getIdAnnee())) {
            System.out.println("L'année " + annee.getIdAnnee() + " existe déjà.");
            return false;
        }

        // Insérer l’année dans la table annees
        ...
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            if (rowsInserted > 0) {
                // Associer l’année à l’étudiant dans etudiants_annees
                String assocQuery = "INSERT INTO etudiants_annees (idEtudiant, idAnnee) VALUES (?, ?)";
                try (PreparedStatement assocStmt = connection.prepareStatement(assocQuery)) {
                    assocStmt.setInt(1, idEtudiant);
                    assocStmt.setInt(2, annee.getIdAnnee());
    ...
    // Récupérer toutes les années, triées par anneeScolaire
    ...
    // Récupérer toutes les années pour un étudiant spécifique
    ...
    // Supprimer une année et ses associations
    public boolean deleteAnnee(int idAnnee) {
        ...
        // Supprimer les associations dans etudiants_annees
        String assocQuery = "DELETE FROM etudiants_annees WHERE idAnnee = ?";
        try (PreparedStatement assocStmt = connection.prepareStatement(assocQuery)) {
            assocStmt.setInt(1, idAnnee);
            assocStmt.executeUpdate();
        } catch (SQLException e) {
            System.err.println("Erreur lors de la suppression des associations : " + e.getMessage());
            return false;
        }
    ...
    public boolean ajouterAnneeEtd(int idEtudiant, Annee annee) throws SQLException {
        if (annee == null) {
        ...
        try {
            ...
            String insertQuery = "INSERT INTO etudiants_annees (idEtudiant, idAnnee) VALUES (?, ?)";
            try (PreparedStatement insertStmt = connection.prepareStatement(insertQuery)) {
                insertStmt.setInt(1, idEtudiant);
                insertStmt.setInt(2, idAnnee);
                System.out.println("Inserting into etudiants_annees: idEtudiant=" + idEtudiant + ", idAnnee=" + idAnnee);
                int rowsAffected = insertStmt.executeUpdate();
                if (rowsAffected == 0) {
                    connection.rollback();
                    System.err.println("Failed to insert into etudiants_annees");
                    return false;
                }
            }
    ...
    // Associer une année à un étudiant
    public boolean associerAnneeAEtudiant(int idEtudiant, int idAnnee) {
        if (!anneeExists("idAnnee", idAnnee)) {
            System.out.println("L'année avec ID " + idAnnee + " n'existe pas.");
            return false;
        }
        ...
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            System.err.println("Erreur lors de l'association de l'année à l'étudiant : " + e.getMessage());
            return false;
        }
    }

    // Dissocier une année d’un étudiant
    public boolean dissocierAnneeDeEtudiant(int idEtudiant, int idAnnee) {
        String query = "DELETE FROM etudiants_annees WHERE idEtudiant = ? AND idAnnee = ?";
        try (PreparedStatement statement = connection.prepareStatement(query)) {
            statement.setInt(1, idEtudiant);
            statement.setInt(2, idAnnee);
            int rowsDeleted = statement.executeUpdate();
            if (rowsDeleted > 0) {
                System.out.println("Année dissociée de l'étudiant avec succès !");
                return true;
            }
        } catch (SQLException e) {
            System.err.println("Erreur lors de la dissociation de l'année : " + e.getMessage());
        }
        return false;
    }
