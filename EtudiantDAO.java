...
public class EtudiantDAO {
    ...
    public boolean ajouterEtudiant(Etudiant etudiant) throws SQLException {
        ...
        try {
            ...
            // 1. First, add the user to the `utilisateurs` table
            ...
            boolean addUser = auth.ajouterUtilisateur(
                new Utilisateur(
                    ...
                    Role.Etudiant // <-- Should be Role.Etudiant, not Admin!
                ),
                etudiant.getMotdepasse()
            );
            ...
            // 2. Handle `idEmploiDuTemps` safely (check for null and existence)
            Integer idEmploiDuTemps = etudiant.getIdEmploiDuTemps();
            if (idEmploiDuTemps != null && idEmploiDuTemps != 0) {
                if (!emploiDuTempsDAO.emploiExiste(idEmploiDuTemps)) {
                    System.out.println("⚠️ idEmploiDuTemps " + idEmploiDuTemps + " n'existe pas, insertion avec NULL.");
                    idEmploiDuTemps = null;
                }
            } else {
                idEmploiDuTemps = null; // Ensure NULL if 0 or null
            }

            // 3. Insert into `etudiants` table
            ...
            try (PreparedStatement etudiantPs = connection.prepareStatement(etudiantSql)) {
                ...
                int rowsAffected = etudiantPs.executeUpdate();
                if (rowsAffected > 0) {
            ...
            System.err.println("❌ Erreur SQL lors de l'ajout de l'étudiant : " + e.getMessage());
            try {
            ...
            try {
                ...
                System.err.println("⚠️ Erreur lors de la restauration de l'auto-commit : " + ex.getMessage());
            }
        }
    }
    ...
    public boolean modifierEtudiant(Etudiant etudiant) throws SQLException {
        ...
        Integer idEmploiDuTemps = etudiant.getIdEmploiDuTemps();
     // Handle null or 0 cases first
     if (idEmploiDuTemps == null || idEmploiDuTemps == 0) {
         idEmploiDuTemps = null;
     } 
     // Then check if it exists in database
     else if (!emploiDuTempsDAO.emploiExiste(idEmploiDuTemps)) {
